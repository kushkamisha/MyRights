# Генерирует биткоин-адрес и приватный ключ из данного файла
# Сохраняет из в файл pr-key.txt
#
...
file = ARGV[0]
passphrase = Base64.encode64(IO.read(file))
...
priv_key_and_version = "\x00".b + priv_key + "\x01".b
...
# сохраняем полученные адрес-ключ в файл
File.open('pr-key.txt', 'a'){ |f| f.write(pub_addr + "\t" + privateWIF + "\r\n") }